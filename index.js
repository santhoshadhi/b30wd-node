...
//db.movies.findone to get movie from database
...
app.get('/movies', async function (req, res) {
  //const data = req.read;
  const moviess = await client.db("b30wd").collection("movies").find({}).toArray();
  res.send(moviess)
})
...
app.get("/movies/:id",async function (request, response) {
  ...
  //const movies = movie.find((data) => data.id === id);//filter return an array and find returns an element
  const movies=await client.db("b30wd").collection("movies").findOne({id:id})
  //response.send(movies)
  movies? response.send(movies): response.status(404).send({ message: "No such movie found 😅" });
...
app.put("/movies/:id", async function (request, response) {
  const { id } = request.params;
  //db.movies.updateOne({id:id}, {$set:updatedData})
  const updatedData = request.body;
 
  const result = await client.db("b30wd").collection("movies").updateOne({ id: id }, { $set: updatedData });
  response.send(result)
});
